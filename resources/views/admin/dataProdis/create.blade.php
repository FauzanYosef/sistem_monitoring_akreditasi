...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.dataProdi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.data-prodis.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_pt') ? 'is-invalid' : '' }}" name="id_pt_id" id="id_pt_id">
                    @foreach($id_pts as $id => $entry)
                        <option value="{{ $id }}" {{ old('id_pt_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('id_pt'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kode_prodi') ? 'is-invalid' : '' }}" name="kode_prodi_id" id="kode_prodi_id">
                    @foreach($kode_prodis as $id => $entry)
                        <option value="{{ $id }}" {{ old('kode_prodi_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenjang_prodi') ? 'is-invalid' : '' }}" name="jenjang_prodi" id="jenjang_prodi">
                    ...
                    @foreach(App\Models\DataProdi::JENJANG_PRODI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenjang_prodi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telp_prodi') ? 'is-invalid' : '' }}" type="text" name="telp_prodi" id="telp_prodi" value="{{ old('telp_prodi', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email_prodi') ? 'is-invalid' : '' }}" type="email" name="email_prodi" id="email_prodi" value="{{ old('email_prodi') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('web_prodi') ? 'is-invalid' : '' }}" type="text" name="web_prodi" id="web_prodi" value="{{ old('web_prodi', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alamat_prodi') ? 'is-invalid' : '' }}" type="text" name="alamat_prodi" id="alamat_prodi" value="{{ old('alamat_prodi', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_prodi') ? 'is-invalid' : '' }}" name="status_prodi" id="status_prodi">
                    ...
                    @foreach(App\Models\DataProdi::STATUS_PRODI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status_prodi', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
