...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.fileupload.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fileuploads.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('namafile') ? 'is-invalid' : '' }}" name="namafile_id" id="namafile_id">
                    @foreach($namafiles as $id => $entry)
                        <option value="{{ $id }}" {{ old('namafile_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
