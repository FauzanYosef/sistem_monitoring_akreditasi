...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.indikatorPenilaian.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.indikator-penilaians.update", [$indikatorPenilaian->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="elemen">{{ trans('cruds.indikatorPenilaian.fields.elemen') }}</label>
                <input class="form-control {{ $errors->has('elemen') ? 'is-invalid' : '' }}" type="text" name="elemen" id="elemen" value="{{ old('elemen', $indikatorPenilaian->elemen) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('elemen_indikator') ? 'is-invalid' : '' }}" name="elemen_indikator" id="elemen_indikator">{{ old('elemen_indikator', $indikatorPenilaian->elemen_indikator) }}</textarea>
