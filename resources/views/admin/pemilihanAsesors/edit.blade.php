...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pemilihanAsesor.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pemilihan-asesors.update", [$pemilihanAsesor->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prodi') ? 'is-invalid' : '' }}" name="prodi_id" id="prodi_id">
                    @foreach($prodis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('prodi_id') ? old('prodi_id') : $pemilihanAsesor->prodi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('prodi'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_assesor') ? 'is-invalid' : '' }}" name="id_assesor_id" id="id_assesor_id">
                    @foreach($id_assesors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_assesor_id') ? old('id_assesor_id') : $pemilihanAsesor->id_assesor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('id_pemilihan') ? 'is-invalid' : '' }}" type="text" name="id_pemilihan" id="id_pemilihan" value="{{ old('id_pemilihan', $pemilihanAsesor->id_pemilihan) }}">
