...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.penilaianIndikator.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.penilaian-indikators.update", [$penilaianIndikator->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('indikator_skor') ? 'is-invalid' : '' }}" name="indikator_skor_id" id="indikator_skor_id">
                    @foreach($indikator_skors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('indikator_skor_id') ? old('indikator_skor_id') : $penilaianIndikator->indikator_skor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('pilihan_penilaian') ? 'is-invalid' : '' }}" name="pilihan_penilaian" id="pilihan_penilaian">{{ old('pilihan_penilaian', $penilaianIndikator->pilihan_penilaian) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('label_nilai') ? 'is-invalid' : '' }}" type="text" name="label_nilai" id="label_nilai" value="{{ old('label_nilai', $penilaianIndikator->label_nilai) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('skor_nilai') ? 'is-invalid' : '' }}" type="number" name="skor_nilai" id="skor_nilai" value="{{ old('skor_nilai', $penilaianIndikator->skor_nilai) }}" step="1">
