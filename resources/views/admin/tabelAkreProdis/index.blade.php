@extends('layouts.admin')
@section('content')
@can('tabel_akre_prodi_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.tabel-akre-prodis.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.tabelAkreProdi.title_singular') }}
            </a>
            <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                {{ trans('global.app_csvImport') }}
            </button>
            @include('csvImport.modal', ['model' => 'TabelAkreProdi', 'route' => 'admin.tabel-akre-prodis.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.tabelAkreProdi.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-TabelAkreProdi">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.id') }}
                        </th>
                        <th>
                            Tahun
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.status_akreditasi') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_doktor') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_magister') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_sarjana') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_profesi_2') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_profesi_1') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_profesi') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_1') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_2') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_4') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_5') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_6') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.jml_vokasi_7') }}
                        </th>
                        <th>
                            {{ trans('cruds.tabelAkreProdi.fields.total') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($tabelAkreProdis as $key => $tabelAkreProdi)
                        <tr data-entry-id="{{ $tabelAkreProdi->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $tabelAkreProdi->id ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->tahun ?? '' }}
                            </td>
                            <td>
                                {{ App\Models\TabelAkreProdi::STATUS_AKREDITASI_SELECT[$tabelAkreProdi->status_akreditasi] ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_doktor ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_magister ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_sarjana ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_profesi_2 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_profesi_1 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_profesi ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_1 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_2 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_4 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_5 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_6 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->jml_vokasi_7 ?? '' }}
                            </td>
                            <td>
                                {{ $tabelAkreProdi->total ?? '' }}
                            </td>
                            <td>
                                @can('tabel_akre_prodi_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.tabel-akre-prodis.show', $tabelAkreProdi->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('tabel_akre_prodi_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.tabel-akre-prodis.edit', $tabelAkreProdi->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('tabel_akre_prodi_delete')
                                    <form action="{{ route('admin.tabel-akre-prodis.destroy', $tabelAkreProdi->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('tabel_akre_prodi_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.tabel-akre-prodis.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 50,
  });
  let table = $('.datatable-TabelAkreProdi:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection