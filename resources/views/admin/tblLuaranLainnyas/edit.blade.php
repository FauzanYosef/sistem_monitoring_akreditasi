...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tblLuaranLainnya.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tbl-luaran-lainnyas.update", [$tblLuaranLainnya->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('hakcipta') ? 'is-invalid' : '' }}" name="hakcipta" id="hakcipta">
                    ...
                    @foreach(App\Models\TblLuaranLainnya::HAKCIPTA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('hakcipta', $tblLuaranLainnya->hakcipta) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('penelitian') ? 'is-invalid' : '' }}" name="penelitian" id="penelitian">{{ old('penelitian', $tblLuaranLainnya->penelitian) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tahun') ? 'is-invalid' : '' }}" type="text" name="tahun" id="tahun" value="{{ old('tahun', $tblLuaranLainnya->tahun) }}">
                @if($errors->has('tahun'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" name="keterangan" id="keterangan">{{ old('keterangan', $tblLuaranLainnya->keterangan) }}</textarea>
