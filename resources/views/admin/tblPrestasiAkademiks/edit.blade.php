...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tblPrestasiAkademik.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tbl-prestasi-akademiks.update", [$tblPrestasiAkademik->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nama_kegiatan">{{ trans('cruds.tblPrestasiAkademik.fields.nama_kegiatan') }}</label>
                <input class="form-control {{ $errors->has('nama_kegiatan') ? 'is-invalid' : '' }}" type="text" name="nama_kegiatan" id="nama_kegiatan" value="{{ old('nama_kegiatan', $tblPrestasiAkademik->nama_kegiatan) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('waktu') ? 'is-invalid' : '' }}" type="text" name="waktu" id="waktu" value="{{ old('waktu', $tblPrestasiAkademik->waktu) }}">
                @if($errors->has('waktu'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tingkat') ? 'is-invalid' : '' }}" name="tingkat" id="tingkat">
                    ...
                    @foreach(App\Models\TblPrestasiAkademik::TINGKAT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tingkat', $tblPrestasiAkademik->tingkat) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('prestasi') ? 'is-invalid' : '' }}" name="prestasi" id="prestasi">{{ old('prestasi', $tblPrestasiAkademik->prestasi) }}</textarea>
