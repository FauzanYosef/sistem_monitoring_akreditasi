...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tblSitasiKarya.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tbl-sitasi-karyas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="nama_penulis">{{ trans('cruds.tblSitasiKarya.fields.nama_penulis') }}</label>
                <input class="form-control {{ $errors->has('nama_penulis') ? 'is-invalid' : '' }}" type="text" name="nama_penulis" id="nama_penulis" value="{{ old('nama_penulis', '') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('artikel') ? 'is-invalid' : '' }}" name="artikel" id="artikel">{{ old('artikel') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jml_artiker') ? 'is-invalid' : '' }}" type="number" name="jml_artiker" id="jml_artiker" value="{{ old('jml_artiker', '') }}" step="1">
